...
void test_self_define_type_traits()
{
    ...
    assert(typeid(stl::type_traits<Item>::has_tirvial_destructor) == typeid(stl::__false_type));
...
void test_basic_type_traits()
{
    ...
    assert(typeid(typename stl::type_traits<T>::has_tirvial_destructor) == typeid(stl::__true_type));
